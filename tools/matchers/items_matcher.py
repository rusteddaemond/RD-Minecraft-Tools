...
Finds duplicate items across different namespaces and generates OEI datapacks
for item matching. Useful for mod compatibility and item unification.

Generates OEI datapack format:
- Datapack: data/oei/replacements/replacements.json

Usage:
    python -m tools.matchers.items_matcher [options]
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Find duplicate items across namespaces and generate OEI datapack",
...
Examples:
  ...
  python -m tools.matchers.items_matcher --config paths.json
  
  # Non-interactive with pre-selected namespace
  python -m tools.matchers.items_matcher --namespace minecraft --no-interactive
  
  # Custom filename
  python -m tools.matchers.items_matcher --filename my_replacements.json
        """
    )
    parser.add_argument(
        "--config",
    ...
    config = MatcherConfig(
        ...
        output_dir=paths.output / "oei_datapack",
        datapack_path="oei",
        match_field="matchItems",
        result_field="resultItems",
